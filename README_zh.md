...
匹配类型。未匹配的显示器会保留完整的普通工作区宽度。窗口规则执行前，会
按命令顺序预判每个 `move --side-area` 执行时所在的显示器，包括此前的工作区
移动。如果请求的侧边区域不可用，或无法安全预判目标显示器，整条规则会在执行
任何命令前暂缓，`run_once` 规则保持待执行状态。交互式执行同一命令时，如果
目标侧边区域不可用，则会返回错误。
